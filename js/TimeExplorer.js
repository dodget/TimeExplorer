...
function padToNDigit(number, nDigits) {
  ...
  const currentLength = str[0] === '-' ? str.length - 1 : str.length;
  const digits = nDigits - (currentLength - 1);
  const insert = str[0] === "-" ? 1 : 0;
  return str.slice(0, insert) + Array(digits).join("0") + str.slice(insert);
}

/**
 * Takes a value and bounds and returns if the value is between the bounds
...
class TimeExplorer {
  ...
  constructDate(year,month,day,hours,minutes){
    ...
    // Set default date
    let date = new Date("0001-01-01T00:00");

    if (year != null)  {
      if (year < 0) { // If BCE date, just set the year
        ...
        let dateString = padToNDigit(year,4) + "-01-01T00:00";
   ...
   * function.
   * @uses constructDate, checkBounds, transformValue, timeParse
  ...
  get_datetime(datum,year_column,month_column,day_column,time_column,callback) {
    ...
    const tempYear = datum[year_column].trim();
    let year = self.transformValue(tempYear, 'year', parseInt)

    const tempMonth = datum[month_column].trim();
    let month = self.transformValue(tempMonth, 'month', parseInt);
    month = checkBounds(month, 1, 12) ? month - 1 : `Invalid month: "${datum[month_column]}"`;

    const tempDay = datum[day_column].trim();
    let day = self.transformValue(tempDay, 'day', parseInt);
    day = checkBounds(day, 1, 31) ? day : `Invalid day: "${datum[day_column]}"`;

    const tempTime = datum[time_column].trim();
    let time = self.transformValue(tempTime, 'time', timeParse);
    let hours = time ? time[0] : null;
    let minutes = time ? time[1] : null;

    let vals = [year,month,day,hours,minutes];
    // Check if there are any out of order non-null date parts or other errors
    ...
    return callback(year,month,day,hours,minutes);
  };

  /**
   * Takes an input, checks it, and passes it through a supplied function.
   * @param {string} val - the value to transform
   * @param {string} input - the input name to return error message with.
   * @param {function} func - the function to process val with.
   */
  transformValue(val, input, func) {
    if (val == null || val == "") {
      return null;
    }
    return func(val) ? func(val) : `Invalid "${input}": "${val}"`;
  }
